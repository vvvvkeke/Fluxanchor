...
def color_for_error(error: float, min_error: float, max_error: float, cmap: LinearSegmentedColormap) -> str:
    ...
    if max_error <= min_error:
        return COLOR_LOW
    # Low error -> dark purple, high error -> light purple
    norm = (error - min_error) / (max_error - min_error)
...
def plot_prism_colorbar():
    """Single-hue error colorbar (low error -> dark purple)."""
    ...
    cbar.set_label('Flux error (low → high)', rotation=270, labelpad=15)
    ax.set_title("Flux error colormap: #825CA6 → #EEE6F5")
    plt.show()
