...
def main(args):
    ...
    elif args.model_name == "biocontext":
    ...
    if args.gpu:
        if not args.multi_gpu:  # 单卡训练
            ...
            model = Model(input_features=512, device=device, model_name=args.model_name).to(device)
            # wandb.init(
            ...
            model = Model(input_features=512, device=device, model_name=args.model_name)
            # 确保 model 中没有 SyncBN
    ...
    if args.rubisco == 'none':
        train_dataloader, kcat_scaler, km_scaler = load_dataset(mode="train", batch_size=int(args.batch_size),
                                                                my_print=my_print, model_name=args.model_name)
        val_dataloader, _, _ = load_dataset(mode="val", batch_size=int(args.batch_size), my_print=my_print,
                                            model_name=args.model_name)
        test_dataloader, _, _ = load_dataset(mode="test", batch_size=int(args.batch_size), my_print=my_print,
                                            model_name=args.model_name)
    elif args.rubisco == 'kcat':
        train_dataloader, val_dataloader, test_dataloader, kcat_scaler, km_scaler = load_rubisco('protein', path="predict/rubisco/dataset")   ### 修改
    elif args.rubisco == 'km':
        train_dataloader, val_dataloader, test_dataloader, kcat_scaler, km_scaler = load_rubisco('protein', path="predict/rubisco_km/dataset")  ### 修改
    ...
    if args.task == "test" or args.task == "continue_train":  # 如果已有权重且开启测试或继续训练模式，则读取权重
        ...
        if (not args.multi_gpu) or (args.multi_gpu and accelerator.is_main_process):
            checkpoint = torch.load(os.path.join(result_path, "last_weight.pt"), map_location=device)
            load_uni_modal_pretrain(model, pth_file=os.path.join(result_path, "last_weight.pt"),
            ...
            if args.task == "continue_train":
                try:
                    ...
                    raise RuntimeError("模型参数与optimizer不严格一致，无法加载optimizer参数继续训练，不知道怎么修这个bug，等死")
            epoch = checkpoint['epoch']
            last_epoch = checkpoint['epoch'] + 1
            train_losses = checkpoint['train_losses']
            val_losses = checkpoint['val_losses']
            early_stopping.val_loss_min = val_losses[-1]
            early_stopping.best_val_loss = val_losses[-1]
    ...
    if args.task != "test":  # 训练模型
        ...
        for epoch in range(last_epoch, int(args.epochs) + last_epoch):
            ...
            val_loss, val_kcat_pcc, val_kcat_p_value, val_km_pcc, val_km_p_value = eval_model(test_dataloader,
                                                                                              model,
    ...
    test_loss, test_kcat_pcc, test_kcat_p_value, test_km_pcc, test_km_p_value, \
        true_kcats, true_kms, pred_kcats, pred_kms, indexes = eval_model(val_dataloader,
                                                                model,
                                                                kcat_scaler, km_scaler,
...
if __name__ == '__main__':
    # 多gpu运行命令： CUDA_VISIBLE_DEVICES=0,1 accelerate launch train_test_model.py --multi_gpu True
    ...
    parser.add_argument('--gpu_id', type=str, default='0', help="single gpu id")
    parser.add_argument("--batch_size", type=str, default="64")  # 16
    ...
    parser.add_argument("--model_name", type=str, default='fusion',
                        choices=['smiles', 'protein', 'fusion', 'biocontext'])
    parser.add_argument("--task", type=str, default='train', choices=['train', 'test', 'continue_train'])
    parser.add_argument("--result_dir", type=str, default='best')  # best
    # 是否使用rubisco数据集进行训练和测试，如果是none则使用普通数据集，否则使用kcat
    parser.add_argument("--rubisco", type=str, default='kcat', choices=['kcat', 'km', 'none'])  # best
