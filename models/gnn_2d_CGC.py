        ...
        for task_idx in range(self.num_tasks):
            ...
            if CrossOver == 0:
                combined_output = torch.cat([
                    shared_task_output_substrate, shared_task_output_product,
                task_specific_output_substrate, task_specific_output_product], dim=-1)  # (batch_size, 64*4=256)
                task_output = self.task_heads[task_idx](combined_output) # (batch_size, 1)
            else:     
                shared_attention = self.cross_attention(shared_task_output_substrate, shared_task_output_product)  # (batch_size, 128)
                task_specific_attention = self.cross_attention(task_specific_output_substrate, task_specific_output_product)  # (batch_size, 128)
                combined_output = torch.cat([shared_attention,task_specific_attention], dim=-1)# (batch_size, 256)
                task_output = self.task_heads[task_idx](combined_output) # (batch_size, 1)
