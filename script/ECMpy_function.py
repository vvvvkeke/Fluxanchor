...
def select_calibration_reaction_by_c13(reaction_kcat_mw_file, c13reaction_file, enzyme_amount, percentage, sigma,
    ...
    This function is used to filter the metabolic reactions that need to adjust kcat values based on C13 flux data. The core logic is to compare the model-predicted flux with the experimentally measured C13 flux data,
    identify those reactions where the model prediction is lower than the experimental value, and determine whether to increase their kcat values to match the experimental data.
    Arguments
    ----------
    ...
    * reaction_kcat_mw_file: The original file storing kcat/mw of reactions.
    * c13reaction_file: The file containing the reaction list which had C13 flux.
    * enzyme_amount: The total enzyme amount (e.g. 0.227).
    * percentage: The percentage which needed change.
    * sigma: The approximated average saturation of enzyme.
    :return: fluxes, kcat, MW and kcat_MW in dataframe.
    """
    reaction_kcat_mw = pd.read_csv(reaction_kcat_mw_file, index_col=0)  # Read kcat and MW data from reaction_kcat_mw_file.
    c13reaction = pd.read_csv(c13reaction_file, index_col=0)  # Read C13 experimental flux data from c13reaction_file.  
    #     print(c13reaction)
    c13reaction_selecet = []
    for index, row in c13reaction.iterrows():
        if index in reaction_kcat_mw.index:
            ...
            ECMpy_c13_reaction_flux = (reaction_kcat_mw.loc[
                                           index, 'kcat_MW'] / 1000) * enzyme_amount * percentage * sigma
            ...
            # If the model predicted flux (enz_model_solution[index]) < C13 experimental flux (row['Flux norm'])
            if enz_model_solution[index] < row['Flux norm']:
                # If the theoretical maximum flux (ECMpy_c13_reaction_flux) < C13 experimental flux, add the reaction to the adjustment list (c13reaction_selecet).
                if ECMpy_c13_reaction_flux < row['Flux norm']:
                    c13reaction_selecet.append(index)
    return (c13reaction_selecet)  # Return the list of reaction IDs that need kcat adjustment c13reaction_selecet.


def get_enz_model_use_c13(reaction_kcat_mw_file, json_model_path, c13reaction_file, percentage,
                          kcat_database_combined_file, model_file, f, ptot, sigma, lowerbound, upperbound,
                          json_output_file, reaction_mw_outfile):
    """Get new enzyme model using C13 reaction to calibration
    This function is used to generate a new enzyme constraint model calibrated with C13 data. It is implemented through the following steps:
        Call select_calibration_reaction_by_c13 to obtain the list of reactions that need adjustment.
        Update the kcat values of these reactions (selecting better values from a larger database).
        Rebuild the enzyme constraint model and save it.
    Arguments
    ----------
    ...
    * c13reaction_file: The file containing the reaction list which had C13 flux.
    * percentage: The percentage which needed change.
    * kcat_database_combined_file: The combined kcat file got from autoPACMEN.
    * model_file: cobra model.
    * f: The enzyme mass fraction.
    * ptot: The total protein fraction in cell.
    * sigma: The approximated average saturation of enzyme.
    ...
    * reaction_mw_outfile: changed file stored reaction kcat/mw.
    ...
    # Call change_reaction_kcat_by_database function to select better kcat values from kcat_database_combined_file.
    change_reaction_list_round1 = change_reaction_kcat_by_database(json_model_path,
                                                                   c13reaction_selecet, kcat_database_combined_file,
                                                                   reaction_kcat_mw_file, reaction_kapp_change_file)
    # Save the updated kcat and MW data to reaction_mw_outfile.
    print('changed reaction: ')
    print(change_reaction_list_round1)
    reaction_kcat_mw_file = reaction_mw_outfile
    # Call trans_model2enz_json_model_split_isoenzyme function to convert the original metabolic model to the new enzyme constraint model.
    ...
    enz_model = get_enzyme_constraint_model(json_output_file)  # Save the new model as a JSON file (json_output_file).
    return enz_model
...
def get_reaction_kcat_mw(model, project_folder, project_name, type_of_default_kcat_selection, enzyme_unit_number_file,
                         json_output_file):
    """Adds proteomic constraints according to sMOMENT to the given stoichiometric model and stores it as SBML.
    ...
    * json_output_file: str ~ The file path to save the output as a JSON file.
    ...
    for model_reaction_id in model_reaction_ids:
        ...
        if model_reaction_id in reaction_mw.keys():
            ...
            reaction_kcat_mw[model_reaction_id]['kcat_MW'] = reaction_kcat_mw[model_reaction_id]['kcat'] * 3600000 / \
                                                             reaction_mw[model_reaction_id]
...
def find_most_dense_value(data, adjust_bandwidth_factor=0.5):
    """
    找到一维数据中最密集区域的代表值（对离群值鲁棒）

    参数：
    data: list 或 np.array - 输入的一维数据
    adjust_bandwidth_factor: float - 带宽调整因子（<1 更敏感，>1 更平滑）
    ...
    try:
        ...
        baseline_bandwidth = kde.covariance_factor()

        # 缩小带宽以更好捕获主分布（可根据数据调整因子）
        kde.set_bandwidth(bw_method=baseline_bandwidth * adjust_bandwidth_factor)

        # 生成评估范围（聚焦于主分布区域，避免离群值干扰）
        q1, q3 = np.percentile(data, [25, 75])
        iqr = q3 - q1
        x = np.linspace(max(np.min(data), q1 - 1.5 * iqr),
                        min(np.max(data), q3 + 1.5 * iqr),
                        1000)

        # 找到密度最高点
        density = kde.evaluate(x)
        kde_peak = x[np.argmax(density)]

        # 在中位数和KDE峰值中选择更接近主簇的值
        main_cluster = data[(data >= q1) & (data <= q3)]
        if len(main_cluster) > 0:
            main_center = np.mean([median, kde_peak])
            return main_center
        else:
            return median
    except:
        # 如果KDE失败（如所有数据相同），退回中位数
        return median
# def change_PSTA(reaction_kcat_mw):
#     for index, row in reaction_kcat_mw.iterrows():
#         if index == "PSTA":
#             break
#     return reaction_kcat_mw
def EkiLLM_trans_model2enz_json_model_split_isoenzyme(ecGEM_path, model_file, reaction_kcat_mw_file, f, ptot, sigma, lowerbound,
                                               upperbound, json_output_file, synauto=False, comdf=None):
    ...
    json_path = f"{ecGEM_path}/{model_name}_irreversible.json"
    ...
    for eachreaction in range(len(dictionary_model['reactions'])):
        ...
        if reaction_id in reaction_kcat_mw.index:
            if reaction_kcat_mw.loc[reaction_id, 'catalytic_efficiency']:
                dictionary_model['reactions'][eachreaction]['catalytic_efficiency'] = reaction_kcat_mw.loc[
                    reaction_id, 'catalytic_efficiency']
                ...
                dictionary_model['reactions'][eachreaction]['method'] = 'cal'
        else:
...
def trans_model2enz_json_model_split_isoenzyme(ecGEM_path, model_file, reaction_kcat_mw_file, f, ptot, sigma, lowerbound,
                                               upperbound, json_output_file, synauto=False, comdf=None):
    ...
    * upperbound:  Upperbound  of enzyme concentration constraint.
    ...
    if synauto:
        ...
        # model.reactions.get_by_id("EX_glyc_e").bounds = (-10, 10)
    convert_to_irreversible(model)
    model = isoenzyme_split(model)
    model_name = model_file.split('/')[-1].split('.')[0]
    save_file = model_file.split('/')[1]
...
def get_enzyme_constraint_model(json_model_file):
    ...
    * json_model_file: json Model file.
    ...
    :return: Construct an enzyme-constrained model (with optional flux ratio constraints).
    """
    # 从JSON文件加载模型字典
    dictionary_model = json_load(json_model_file)
    # 使用COBRApy的JSON加载器创建代谢模型对象
    ...
    for rxn in model.reactions:
        ...
        for eachr in dictionary_model['reactions']:
            # 当找到匹配的反应ID时
            if rxn.id == eachr['id']:
                # 检查该反应是否有kcat_MW值（酶催化效率与分子量的比值）
                if eachr['kcat_MW']:
                    # 计算反应的酶约束系数（1/kcat_MW）并存储到字典中
                    # 这个系数表示每单位反应通量需要的酶量
                    coefficients[rxn.forward_variable] = 1 / float(eachr['kcat_MW'])
                # 找到匹配后跳出内层循环，继续处理下一个反应
                break
    ...
    # 创建一个新的约束条件，初始值为0，设置上下限
    # 这个约束表示所有反应所需的总酶量必须在指定范围内
    constraint = model.problem.Constraint(0, lb=lowerbound, ub=upperbound)
    # 将约束添加到模型中
    model.add_cons_vars(constraint)
    # 更新求解器以应用新添加的约束
    model.solver.update()
    # 设置约束的线性系数，将每个反应变量与其酶约束系数关联起来
    constraint.set_linear_coefficients(coefficients=coefficients)

    # # 可选：添加 flux ratio 约束
    # if flux_ratio_file is not None:
    #     import sys
    #     sys.path.append(r'./')
    #     from my_flux_ratio_test import add_flux_ratio_constraints
    #     model, constraints_added = add_flux_ratio_constraints(model, flux_ratio_file)
    #     print(f"已添加 {constraints_added} 个 flux ratio 约束")

    # 返回添加了酶约束（和可选的 flux ratio 约束）的模型
    return model

def get_enzyme_constraint_model_for_dfba(json_model_file):
    """using enzyme concentration constraint
    json model to create a COBRApy model.
    ...
    * json_model_file: json Model file.
    ...
    constraint.set_linear_coefficients(coefficients=coefficients)
    # # 保存约束信息到模型属性，但不保存到文件
    # model._temp_enzyme_constraint = constraint
    # model._temp_enzyme_coefficients = coefficients
    return model
            ...
            if index == eachreaction['id']:
                if 'annotation' in eachreaction.keys():
                    if 'ec-code' in eachreaction['annotation'].keys():
                        if isinstance(eachreaction['annotation']['ec-code'], list):
                            model_pfba_solution_detail.loc[index, 'ec-code'] = (',').join(
                                eachreaction['annotation']['ec-code'])
                        else:
                ...
                if 'kcat_MW' in eachreaction.keys():
                    if eachreaction['kcat_MW']:
                        ...
                        model_pfba_solution_detail.loc[index, 'E'] = float(row['fluxes']) / float(
                            eachreaction['kcat_MW'])
                break
...
def GENEID_2_ACC_from_uniprot_new(query_list, strain_name, gene_uniprot_outfile):  #
    with open(gene_uniprot_outfile, 'w') as f:
        ...
        for query in query_list:
            url = 'https://rest.uniprot.org/uniprotkb/search?query=accession:%s AND organism_name: %s &format=tsv&fields=accession,id,protein_name,gene_names,organism_name,ec,mass,sequence,go_id,xref_kegg' % (
                query, strain_name)
...
def get_model_substrate_obj(use_model):
    ...
    此函数用于从代谢模型中提取以下信息：
        目标反应（生物量反应）：模型中优化目标（如生物量合成）。
        底物列表：当前模型中允许摄取的主要底物（如碳源）。
        底物浓度约束：对应底物的最大摄取速率。
        是否存在 ATP 维持需求（ATPM）：标记模型是否需要基础 ATP 维持。
    '''

    ATPM = 'No'
    substrate_list = []
    concentration_list = []
    EX_exclude_reaction_list = ['EX_pi_e', 'EX_h_e', 'EX_fe3_e', 'EX_mn2_e', 'EX_co2_e', 'EX_fe2_e', 'EX_h2_e',
                                'EX_zn2_e', \
                                'EX_mg2_e', 'EX_ca2_e', 'EX_so3_e', 'EX_ni2_e', 'EX_no_e', 'EX_cu2_e', 'EX_hg2_e',
                                'EX_cd2_e', \
                                'EX_h2o2_e', 'EX_h2o_e', 'EX_no2_e', 'EX_nh4_e', 'EX_so4_e', 'EX_k_e', 'EX_na1_e',
                                'EX_o2_e', \
                                'EX_o2s_e', 'EX_ag_e', 'EX_cu_e', 'EX_so2_e', 'EX_cl_e', 'EX_n2o_e', 'EX_cs1_e',
                                'EX_cobalt2_e']
    ...
    for r in use_model.reactions:
        ...
        elif not r.upper_bound == 0 and not r.upper_bound == 1000 and not r.upper_bound == 999999 and abs(
                r.upper_bound) > 0.1:  # 排除很小的值
...
def parse_sabio_rk_for_eclist(ec_numbers_list: List[str], json_output_path: str,
...
def get_protein_mass_mapping_from_local(sbml_path: str, project_folder: str, project_name: str,
                                        uniprot_data_file: str) -> None:
...
def change_reaction_kcat_by_database(json_model_path, select_reactionlist, EC_max_file, reaction_kcat_mw_file,
                                     reaction_kapp_change_file):
            ...
            if isinstance(ec_number, str):
                if ec_number in Brenda_sabio_combined_select.keys():
                    ...
                    if reaction_kcat_mw.loc[eachreaction, 'kcat'] < reaction_kcat_max:
                        ...
                        reaction_kcat_mw.loc[eachreaction, 'kcat_MW'] = reaction_kcat_max * 3600 * 1000 / \
                                                                        reaction_kcat_mw.loc[eachreaction, 'MW']
...
def get_enz_model_use_enz_usage_by_eckcat(enz_ratio, json_model_path, reaction_flux_file, EC_max_file,
                                          reaction_kcat_mw_file, f, \
    ...
    * reaction_mw_outfile: changed file stored reaction kcat/mw.
    ...
    change_reaction_list_round1 = change_reaction_kcat_by_database(json_model_path, select_reaction, EC_max_file,
                                                                   reaction_kcat_mw_file, reaction_mw_outfile)
    print('changed reaction: ')
    print(change_reaction_list_round1)
...
def adj_reaction_kcat_by_database(json_model_path, select_reactionlist, need_change_reaction_list,
                                  changed_reaction_list, EC_max_file, reaction_kcat_mw):
    """Use the kcat in database to change reaction kcat in model
...
def change_enz_model_by_enz_usage(json_model_path, reaction_flux_file, EC_max_file, reaction_kcat_mw,
                                  need_change_reaction_list, changed_reaction_list, f, ptot, sigma, lowerbound,
                                  upperbound, json_output_file):
    """Get new enzyme model using enzyme usage to calibration
    ...
    * reaction_mw_outfile: changed file stored reaction kcat/mw.
    ...
    print('Need changing reaction: ')
    print(select_reaction)
    [need_change_reaction_list, changed_reaction_list, reaction_kcat_mw] = adj_reaction_kcat_by_database(
        json_model_path, [select_reaction], need_change_reaction_list, changed_reaction_list, EC_max_file,
        reaction_kcat_mw)
    print('Changed reaction: ')
    print(changed_reaction_list)

    adj_trans_model2enz_model(json_model_path, reaction_kcat_mw, f, ptot, sigma, lowerbound, upperbound,
                              json_output_file)
...
def get_met_bigg_id(model):
    ...
    * metdf_unique: pandas.DataFrame ~ A DataFrame containing unique metabolite IDs and names.
    if '_c' in str(compound):  # 去掉胞内、胞外和膜间反应
            compound = str(compound).split('_c')[0]
        elif '_e' in str(compound):
            compound = str(compound).split('_e')[0]
        elif '_l' in str(compound):
            compound = str(compound).split('_l')[0]
        elif '_p' in str(compound):
            compound = str(compound).split('_p')[0]
        elif '_u' in str(compound):
            compound = str(compound).split('_y')[0]
        elif '_x' in str(compound):
            compound = str(compound).split('_x')[0]
        elif '_y' in str(compound):
            compound = str(compound).split('_y')[0]
    '''

    metlist = []
    metnamelist = []
    for met in model.metabolites:  # Why is '2ddg6p_c' metabolite not in model.metabolites?
        if str(met).endswith("_c"):
            metlist.append(str(met.id).split('_c')[0])
            metnamelist.append(str(met.name))
        elif str(met).endswith("_e"):
            metlist.append(str(met.id).split('_e')[0])
            metnamelist.append(str(met.name))
        elif str(met).endswith("_l"):
            metlist.append(str(met.id).split('_l')[0])
            metnamelist.append(str(met.name))
        elif str(met).endswith("_p"):
            metlist.append(str(met.id).split('_p')[0])
            metnamelist.append(str(met.name))
        elif str(met).endswith("_u"):
            metlist.append(str(met.id).split('_u')[0])
            metnamelist.append(str(met.name))
        elif str(met).endswith("_x"):
            metlist.append(str(met.id).split('_x')[0])
            metnamelist.append(str(met.name))
        elif str(met).endswith("_y"):
            metlist.append(str(met.id).split('_y')[0])
            metnamelist.append(str(met.name))
        else:
    ...
    # metlist.append('2ddg6p')
    # metnamelist.append('2-Dehydro-3-deoxy-D-gluconate 6-phosphate')
    metdf['met'] = metlist
            ...
            if len(clist) == 0:
                ...
                try:
                    if compound.canonical_smiles is None:
                        ...
                        inchkeydf.loc[eachc, 'Canonical_SMILES'] = canonical_smiles
...
def get_model_protein_sequence_and_mass(model, subbnumdf, prodf_file):
    ...
    * prodf: pandas.DataFrame ~ DataFrame with protein information.
    ...
    prodf['mass'] = prodf['aaseq'].apply(
        lambda seq: ProteinAnalysis(seq.replace('X', ''), monoisotopic=False).molecular_weight())
...
def get_currency_metabolites():
    ...
    # 原始化学种类（不带 compartment 后缀）
    base_currency = [
        'atp', 'adp', 'amp', 'utp', 'udp', 'ump', 'ctp', 'cdp', 'cmp',
        'gtp', 'gdp', 'gmp', 'itp', 'idp', 'imp', 'xmp', 'xdp', 'xtp',
        'datp', 'dadp', 'damp', 'dctp', 'dcdp', 'dcmp', 'dgtp', 'dgdp', 'dgmp',
        'dttp', 'dtdp', 'dtmp', 'ditp', 'didp', 'dimp', 'dutp', 'dudp', 'dump',
        'coa', 'nad', 'nadh', 'nadp', 'nadph', 'fad', 'fadh2',
        'pi', 'ppi', 'pppi',
        'h2o', 'h2o2', 'h', 'h2',
        'co2', 'o2', 'no', 'no2', 'no3', 'n2o',
        'nh3', 'nh4',
        'so2', 'so3', 'so4', 'h2s', 'o2s',
        'q8', 'q8h2',
        'na1', 'k', 'mg2', 'ca2', 'cl',
        'fe2', 'fe3', 'zn2', 'mn2', 'cu', 'cu2', 'cd2', 'hg2', 'ag', 'cobalt2'
    ]

    # 带 compartment 后缀的版本
    expanded_currency = []
    for met in base_currency:
        for comp in compartments.keys():
            expanded_currency.append(f"{met}_{comp}")

    # 返回 set，避免重复
    return set(base_currency + expanded_currency)
...
#     '''
#     currencylist1 = ['coa_c', 'co2_c', 'co2_e', 'co2_p', 'cobalt2_c', 'cobalt2_e', 'cobalt2_p', 'h_c', 'h_e', 'h_p',
#                      'h2_c', 'h2_e', 'h2_p', 'h2o_c', 'h2o_e', 'h2o_p', 'h2o2_c', 'h2o2_e', 'h2o2_p', 'nh4_c', 'nh4_e',
#                      'nh4_p', 'o2_c', 'o2_e', 'o2_p', 'pi_c', 'pi_e', 'pi_p', 'ppi_c', 'pppi_c', 'q8_c', 'q8h2_c',
#                      'no_p', 'no3_p', 'no3_e', 'no_c', 'no2_c', 'no_e', 'no3_c', 'no2_e', 'no2_p', 'n2o_c', 'n2o_e',
#                      'n2o_p', 'h2s_c', 'so3_c', 'so3_p', 'o2s_c', 'h2s_p', 'so2_e', 'so4_e', 'h2s_e', 'o2s_p', 'o2s_e',
#                      'so4_c', 'so4_p', 'so3_e', 'so2_c', 'so2_p', 'ag_c', 'ag_e', 'ag_p', 'na1_c', 'na1_e', 'na1_p',
#                      'ca2_c', 'ca2_e', 'ca2_p', 'cl_c', 'cl_e', 'cl_p', 'cd2_c', 'cd2_e', 'cd2_p', 'cu_c', 'cu_e',
#                      'cu_p', 'cu2_c', 'cu2_e', 'cu2_p', 'fe2_c', 'fe2_e', 'fe2_p', 'fe3_c', 'fe3_e', 'fe3_p', 'hg2_c',
#                      'hg2_e', 'hg2_p', 'k_c', 'k_e', 'k_p', 'mg2_c', 'mg2_e', 'mg2_p', 'mn2_c', 'mn2_e', 'mn2_p',
#                      'zn2_c', 'zn2_e', 'zn2_p', 'nh3']
#     currencylist2 = ['amp_c', 'amp_e', 'amp_p', 'adp_c', 'adp_e', 'adp_p', 'atp_c', 'atp_e', 'atp_p', 'cmp_c', 'cmp_e',
#                      'cmp_p', 'cdp_c', 'cdp_e', 'cdp_p', 'ctp_c', 'ctp_e', 'ctp_p', 'gmp_c', 'gmp_e', 'gmp_p', 'gdp_c',
#                      'gdp_e', 'gdp_p', 'gtp_c', 'gtp_e', 'gtp_p', 'imp_c', 'imp_e', 'imp_p', 'idp_c', 'idp_e', 'idp_p',
#                      'itp_c', 'itp_e', 'itp_p', 'ump_c', 'ump_e', 'ump_p', 'udp_c', 'udp_e', 'udp_p', 'utp_c', 'utp_e',
#                      'utp_p', 'xmp_e', 'xmp_c', 'xmp_p', 'xdp_c', 'xdp_e', 'xdp_p', 'xtp_c', 'xtp_e', 'xtp_p', 'damp_c',
#                      'damp_e', 'damp_p', 'dadp_c', 'dadp_e', 'dadp_p', 'datp_c', 'datp_e', 'datp_p', 'dcmp_c', 'dcmp_e',
#                      'dcmp_p', 'dcdp_c', 'dcdp_e', 'dcdp_p', 'dctp_c', 'dctp_e', 'dctp_p', 'dgmp_c', 'dgmp_e', 'dgmp_p',
#                      'dgdp_c', 'dgdp_e', 'dgdp_p', 'dgtp_c', 'dgtp_e', 'dgtp_p', 'dimp_c', 'dimp_e', 'dimp_p', 'didp_c',
#                      'didp_e', 'didp_p', 'ditp_c', 'ditp_e', 'ditp_p', 'dump_c', 'dump_e', 'dump_p', 'dudp_c', 'dudp_e',
#                      'dudp_p', 'dutp_c', 'dutp_e', 'dutp_p', 'dtmp_c', 'dtmp_e', 'dtmp_p', 'dtdp_c', 'dtdp_e', 'dtdp_p',
#                      'dttp_c', 'dttp_e', 'dttp_c', 'fad_c', 'fad_p', 'fad_e', 'fadh2_c', 'fadh2_e', 'fadh2_p', 'nad_c',
#                      'nad_e', 'nad_p', 'nadh_c', 'nadh_e', 'nadh_p', 'nadp_c', 'nadp_e', 'nadp_p', 'nadph_c', 'nadph_e',
#                      'nadph_p']
#     currencylist3 = ['cdp', 'ag', 'dctp', 'dutp', 'ctp', 'gdp', 'gtp', 'ump', 'ca2', 'h2o', 'datp', 'co2', 'no2', 'no',
#                      'k', 'zn2', 'no3', 'o2', 'cl', 'udp', 'damp', 'ditp', 'dump', 'q8h2', 'pppi', 'idp', 'dimp', 'pi',
#                      'dttp', 'so4', 'adp', 'xtp', 'dgtp', 'dadp', 'coa', 'ppi', 'h2', 'cmp', 'fe2', 'o2s', 'h', 'gmp',
#                      'itp', 'q8', 'cobalt2', 'n2o', 'xmp', 'xdp', 'nadph', 'cu', 'cu2', 'atp', 'dgmp', 'imp', 'h2s',
#                      'utp', 'dtmp', 'fadh2', 'so3', 'fad', 'cd2', 'dgdp', 'nad', 'nadh', 'hg2', 'dcmp', 'dudp', 'dtdp',
#                      'didp', 'mn2', 'dcdp', 'nh4', 'amp', 'fe3', 'nadp', 'so2', 'h2o2', 'mg2']
...
def extract_metabolite_id(met_id):
    ...
    if met_id.endswith('_c'):
        ...
        return met_id.split('_p')[0]
    elif met_id.endswith('_e'):
        return met_id.split('_e')[0]
    elif met_id.endswith('_l'):
        return met_id.split('_l')[0]
    elif met_id.endswith('_u'):
        return met_id.split('_u')[0]
    elif met_id.endswith('_x'):
        return met_id.split('_x')[0]
    elif met_id.endswith('_y'):
        return met_id.split('_y')[0]
    return met_id
...
def preprocess_metabolites(model, currency_metabolites):
    ...
    * metdf: pandas.DataFrame ~ DataFrame with preprocessed metabolite information.
    ...
    # reactants and products are all metabolites involved in the reaction
    for reaction in model.reactions:
        if reaction.gene_reaction_rule and len(reaction.reactants) + len(reaction.products) > 1:
...
def DL_kcat_mw_calculation(DLouputdf, metdf):
    ...
    * metdf: pandas.DataFrame - DataFrame containing metabolite, reaction, gene, similes, prosequence, mass information.
    ...
    DLoutputdf_rex = DLoutputdf_rex.sort_values('Kcat value (1/s)', ascending=False).drop_duplicates(
...
def get_subunit_number(sub_description_path, gene_subnum_path):
    ...
    * df: pd.DataFrame - DataFrame with subunit numbers for each gene.
    ...
    for row in df.itertuples():
        ...
        sub_dict = {
            'Monomer.': '1', 'Homodimer.': '2', 'Homodimerizes.': '2', 'Homotrimer.': '3', 'Homotetramer.': '4',
            'Homopentamer.': '5', 'Homohexamer.': '6', 'Homoheptamer.': '7',
            'Homooctamer.': '8', 'Octamer.': '8', 'Homodecamer.': '10', 'Homododecamer.': '12',
            'Active as a monomer': '1', 'Binds DNA as a monomer': '1', "Monomer \(in vitro\) \(PubMed:": '1',
            "Monomer \(PubMed:": '1', 'Monomer \(Ref.': '1',
            'Monomer in solution': '1', 'Monomer;': '1', 'Monomeric in solution.': '1', 'Binds DNA as monomer': '1',
            'Forms monomers in solution': '1', 'Monomer \(disintegrin\)': '1',
            'Monomer \(G-actin\)': '1', 'Dimer of': '1', 'Heterodimer': '1', 'heterodimer': '1', 'Heterotrimer': '1',
            'Composed of two chains': '2',
            'Forms head-to-head homodimers': '2', 'Forms head-to-tail homodimers': '2',
            'Forms homodimer in solution': '2',
            'Forms homodimers': '2', 'Homodimer \(PubMed:': '2', 'Homodimer \(Ref.': '2', 'Homodimer \(via': '2',
            'Homodimer in solution': '2', 'Homodimer of': '2', 'Homodimer,': '2',
            'Homodimer;': '2', 'Forms a homodimer': '2', 'Active as a homodimer.': '2', 'Acts as homodimer': '2',
            'Binds DNA as a homodimer': '2', 'Binds DNA as homodimer': '2',
            'Can form homodimer': '2', 'Can homodimerize': '2', 'Forms a functional homodimer': '2',
            'Forms an asymmetric homodimer': '2', 'Forms disulfide-linked homodimers': '2',
            'Forms homodimer': '2', 'Head to tail homodimer': '2', 'Headphone-shaped homodimer': '2',
            'Homodimer \(in vitro\)': '2', 'Homodimer formed by': '2', 'Homodimer in': '2',
            'Homodimer that': '2', 'Homodimers.': '2', 'homodimers': '2', 'homodimer around DNA': '2',
            'Tetramer of': '2', 'heterotetramer': '2',
            'Homotrimer': '3', 'Homotrimer,': '3', 'Homotrimer;': '3', 'Forms homotrimers \(PubMed:': '3',
            'Homotrimer formed of': '3', 'Homotrimer in solution': '3',
            'Can form homotrimer': '3', 'Forms homotrimers': '3', 'Homotrimer \(PubMed:': '3', 'Homotrimers of': '3',
            'Forms homotetramers': '4', 'Homotetramer,': '4',
            'Homotetramer composed of': '4', 'Homotetramer consisting of': '4', 'Homotetramer formed': '4',
            'Homotetramer in solution': '4', 'Homotetramer:': '4', 'Homotetramer;': '4',
            'A homotetramer formed': '4', 'Binds DNA as a homotetramer': '4', 'Homotetramer \(in vitro\)': '4',
            'Homotetramer \(MAT-I\)': '4', 'Homotetramer \(PubMed:': '4', 'Heterooctamer': '4',
            'Homotetramer \(Ref.': '4', 'Homotetramer in': '4', 'Homopentamer \(PubMed:': '5', 'Homopentamer with': '5',
            'Homopentamer arranged in': '5',
            'Homopentamer;': '5', 'Forms a homopentamer': '5', 'Homopentamer \(in vitro\)': '5', 'Homopentamer,': '5',
            'Homohexameric': '6', 'Homohexamer': '6',
            'Homohexamer \(PubMed:': '6', 'Homohexamer composed of': '6', 'Homohexamer in solution': '6',
            'Homohexamer with': '6', 'Homohexamer,': '6', 'Homohexamer;': '6',
            'Homohexameric ring arranged as': '6', 'A double ring-shaped homohexamer of': '6',
            'Forms a homohexamer': '6', 'Forms homohexameric rings': '6', 'Forms homohexamers': '6',
            'Homohexamer \(dimer of homotrimers\)': '6', 'Homoheptamer arranged in': '7', 'Homoheptamer;': '7',
            'Heterooligomer': '7', 'Forms only homooctamers': '8',
            'Homooctamer composed of': '8', 'Homooctamer,': '8', 'Homooctamer;': '8', 'Homooctamer \(isoform 2\)': '8',
            'Homooctamer formed by': '8', 'Homooctamer of': '8',
            'Homooctomer \(PubMed:': '8', 'Homodecamer;': '10', 'Homodecamer composed of': '10',
            'Forms an asymmetric tunnel-fold homodecamer': '10', 'Homodecamer,': '10',
            'Homodecamer consisting of': '10', 'Homodecamer of': '10', 'Homodecamer; composed of': '10',
            'Homododecamer \(PubMed:': '12', 'Homododecamer composed of': '12',
            'Homododecamer;': '12', 'Heterotetramer': '2', 'Dodecamer': '12', 'Oligomer of 12 subunits': '12',
            '24-polypeptide': '24', '24 subunits': '24',
            'ClpP subunits': '7', '7 subunits': '7', 'Tat system': '1', 'homodecamer,': '10', 'dodecamer': '12',
            'octamer': '8', 'cylinder': '7',
...
def set_auto_model(model):
    model.reactions.EX_glc__D_e.bounds = (0, 0)  # 禁止葡萄糖摄入
    # model.reactions.EX_fru_e.bounds = (0, 0)  # 禁止果糖摄入
    # model.reactions.EX_glcglyc_e.bounds = (0, 0)  # 禁止甘油摄入
    # model.reactions.EX_sucr_e.bounds = (0, 0)  # 禁止蔗糖摄入
    # 设置唯一氮源硝酸[-0.008,-0.05]
    ...
    # 设置唯一碳源二氧化碳
    model.reactions.get_by_id('EX_co2_e').lower_bound = -100  # 设置二氧化碳的摄入量 [-0.001, -0.25]
    # model.reactions.get_by_id('EX_hco3_e').lower_bound = -5  # 设置二氧化碳的摄入量 [-0.001, -0.25]
    return model

def get_PhPP_data(model_file, model_type, obj, substrate_bound,
                  co2_bound, number, outputfile, x_id, z_id):
    '''
    ...
    * co2_bound: float - Upper bound for the oxygen constraint.
    ...
    * df1: pd.DataFrame - DataFrame containing the results of PhPP analysis.
    ...
    # 使用 numpy.linspace 生成从 1 到 substrate_bound 和从 1 到 co2_bound 的等间距列表。
    # exlistn = np.linspace(no3_lower_bound, substrate_bound, number + 1).round(4).tolist()
    # exlistmo2 = np.linspace(co2_lower_bound, co2_bound, number + 1).round(4).tolist()
    exlistn = np.linspace(1, substrate_bound, number + 1).round(4).tolist()
    exlistmo2 = np.linspace(1, co2_bound, number + 1).round(4).tolist()
    # 创建一个 (number + 1) × (number + 1) 的空 DataFrame，用于存储每个条件下的目标反应通量。
    df1 = pd.DataFrame(index=range(number + 1), columns=range(number + 1))

    for v, i in enumerate(exlistmo2[1:], start=1): # 外层循环遍历氧气摄取速率（exlistmo2）。
        condi = i
        for k, j in enumerate(exlistn[1:], start=1): # 内层循环遍历底物摄取速率（exlistn）。跳过第一个点（1），因为通常第一个点的通量为零。
            condj = j

            # Load the model
            if model_type == 'GEM':
                model = cobra.io.json.load_json_model(model_file)
                # set_auto_model(model)
            else:
...
def drawphpp(GEM_glc_o2_df, ecGEM_glc_o2_df, substrate_bound, o2_bound, obj_bound, number, PhPP_output_fig_file):
    ...
    * fig: go.Figure - Generated 3D surface plot figure.
    '''
    # 此函数用于执行代谢模型的 表型相平面分析（Phenotypic Phase Plane, PhPP），通过系统地调整两种底物（如碳源和氧气）的供应水平，
    # 计算目标代谢物（如生物量）的产率，生成二维通量分布数据。以下是详细分析：
...
def draw_3d_rbas(z_data, substrate_bound, o2_bound, obj_bound, number, PhPP_output_fig_file, x_label, z_label):
    '''
    ...
    * fig: go.Figure - Generated 3D plot figure.
    ...
    x_values = np.linspace(1, o2_bound, number)
    y_values = np.linspace(1, substrate_bound, number)
    # x_values = np.linspace(no3_lower_bound, substrate_bound, number + 1).round(4).tolist()
    # y_values = np.linspace(co2_lower_bound, o2_bound, number + 1).round(4).tolist()
    ...
    layout = go.Layout(
        ...
        scene=dict(
            xaxis=dict(
                ...
                backgroundcolor="lightgrey",
                title=dict(text=f"<b>{z_label} uptake rates<br>(mmol/gDW/h)</b>",
            ...
            yaxis=dict(
                ...
                tickfont=dict(size=15, family='Times New Roman'),
                backgroundcolor="lightgrey",
                title=dict(text=f"<b>{x_label} uptake rates<br>(mmol/gDW/h)</b>",
                           font=dict(size=20, family='Times New Roman'))
            ),
            zaxis=dict(
                range=[0, obj_bound],
...
def draw_3d_rbas2(z_data, substrate_bound, o2_bound, obj_bound, number, PhPP_output_fig_file, x_label, z_label):
    x_values = np.linspace(1, o2_bound, number)
    y_values = np.linspace(1, substrate_bound, number)
    # plt.rcParams['svg.fonttype'] = 'none'
    # plt.rcParams['pdf.fonttype'] = 42
    # 设置全局字体（Adobe Illustrator兼容字体，如Arial、Times New Roman）
    pio.templates.default = "plotly_white"
    pio.templates["plotly_white"].layout.font.family = "Arial"  # 确保字体可嵌入

    # 强制Kaleido引擎生成纯矢量PDF（避免栅格化）
    pio.kaleido.scope.default_format = "pdf"
    pio.kaleido.scope.default_scale = 1  # 保持原始分辨率

    layout = go.Layout(

        template="none",
        plot_bgcolor='white',  # 改为白色背景
        scene=dict(
            xaxis=dict(
                range=[0, o2_bound],
                tickfont=dict(size=15, family='Times New Roman'),
                backgroundcolor="white",  # 改为白色
                gridcolor='lightgray',    # 添加网格颜色
                showgrid=False,
                linewidth=3,  # 坐标轴线宽度
                # title=dict(text=f"<b>{z_label} uptake rates(mmol/gDW/h)</b>",
                #            font=dict(size=20, family='Arial')),
                title=dict(
                    text=f"<b>O<sub>2</sub> uptake rates(mmol/gDW/h)</b>",
                    font=dict(size=20, family='Arial')
                ),

                # title=dict(
                #     text=f"<b>CO<sub>2</sub> uptake rates(mmol/gDW/h)</b>",
                #              font=dict(size=20, family='Arial')
                # ),
                # ticklen=20,  # 增加刻度线长度，使标签远离坐标轴
            ...
            yaxis=dict(
                ...
                linewidth=3,             # 坐标轴线宽度
                title=dict(
                    text=f"<b>\u200B<br><br>{x_label} uptake rates(mmol/gDW/h)</b>",
                    font=dict(size=20, family='Times New Roman')
                ),
                # ticklen=20,  # 增加刻度线长度，使标签远离坐标轴
            ...
            zaxis=dict(
                ...
                tickfont=dict(size=15, family='Times New Roman'),
                backgroundcolor="white",  # 改为白色
                gridcolor='lightgray',    # 改为浅灰色网格
                showgrid=False,
                linewidth=3,             # 坐标轴线宽度
                title=dict(text="<b>Growth rates (h<sup>-1</sup>)</b>",
                           font=dict(size=20, family='Arial')),
                # 隐藏z轴的0刻度
                # tickvals=[v for v in np.linspace(0, obj_bound, 2) if v != 0],
                # ticktext=[str(int(v)) if v != 0 else '' for v in np.linspace(0, obj_bound, 2) if v != 0],
                # title=dict(text="<b>Growth rates<br>(1/h)</b>",
                #            font=dict(size=20, family='Arial')),
                # ticklen=20,  # 增加刻度线长度，使标签远离坐标轴
            )
        ...
        scene_camera_eye=dict(
            x=-2,  # 调整x轴观察角度 (默认-0.8)
            y=-2.1,  # 调整y轴观察角度 (默认-2.1)
            z=0.3  # 调整俯视/仰视角度 (默认0.3)
        ),
        ## e.coli
        # scene_camera_eye=dict(
        #     x=-2,  # 调整x轴观察角度 (默认-0.8)
        ...
        #     z=0.3  # 调整俯视/仰视角度 (默认0.3)
        # ),
        width=1400,  # 从850增加到1200
        height=1200,  # 从850增加到1200
        margin=dict(l=10, r=10, b=10, t=10)  # 减少边距
    )
    # viridis  yes
    # plasma   no
    # inferno  no
    ...
    fig = go.Figure(data=[go.Surface(
        ...
        y=y_values,
        z=z_data.values,
        opacity=0.8,  # 设置透明度，0是完全透明，1是完全不透明
    ...
    #     opacity=0.8,
    #     colorscale='Viridis',
    #     surfacecolor=z_data.values,  # 设置点的颜色
    #     showscale=True,
    #     contours=dict(
    #         x=dict(show=False),
    #         y=dict(show=False),
    ...
    #     x=x_values.repeat(len(y_values)),
    ...
    #         project_z=True,
    #         # opacity=0.8  # 等高线投影的透明度
    #     )
    # )

    fig.write_image(
        PhPP_output_fig_file,
        format='pdf',
        scale=2,  # 默认1，2表示双倍分辨率
        engine="kaleido",
        height=1000,
        width=1200
    )
...
def calculate_yield(model, use_substrate, substrate_name, glc_concentration, columns, correspond_rxn, yield_list):
    ...
    * yield_list: pd.DataFrame - DataFrame with updated yield values.
    '''
    # 限制底物摄取速率为 glc_concentration（如葡萄糖浓度为10 mmol/gDW/h时，设置 (-10, 0)）。
    # 完全禁止底物通过反向反应分泌（如防止葡萄糖被排出）。
    # try:
    ...
    # else:
    #     try:
    #         model.reactions.get_by_id(use_substrate + '_reverse').bounds = (0, 0)  # 完全禁止底物的分泌
    #     except:
    #         pass
    ...
    model.reactions.get_by_id(use_substrate + '_reverse').bounds = (0, glc_concentration)  # 设置底物吸收量
    model.reactions.get_by_id("BIOMASS_Ec_iJO1366_core_53p95M").bounds = (0, 1000)  
    # model.reactions.get_by_id("EX_ac").bounds = (0, 1000)
    pfba_solution = cobra.flux_analysis.pfba(model)
    # pfba_solution = model.optimize()
    for column in range(len(columns)):
        # if columns[column] == "ecoli_biomass":
        #     yield_list.loc[glc_concentration, columns[column]] = pfba_solution.objective_value
...
def draw_overflow_fig(GEMyield_list, ecGEMyield_list, column_list, y_axis_loc_list, color_list, substrate_name,
                      substrate_bound, obj_bound, secrate_bound, trade_off_biomass_yield_figfile):
    '''
    ...
    * fig: go.Figure - Plotly figure object.
    ...
    for column in range(len(column_list)):
        if y_axis_loc_list[column] == 'left':
            trace1 = create_trace(GEMyield_list[substrate_name], GEMyield_list[column_list[column]],
                                  '%s_GEM' % column_list[column], color_list[column], symbol=5, size=10, width=3)
            data1.append(trace1)
            trace2 = create_trace(ecGEMyield_list[substrate_name], ecGEMyield_list[column_list[column]],
                                  '%s_ecGEM' % column_list[column], color_list[len(column_list) + column], symbol=5,
                                  size=10, width=3)
            data1.append(trace2)
        else:
            trace1 = create_trace(GEMyield_list[substrate_name], GEMyield_list[column_list[column]],
                                  '%s_GEM' % column_list[column], color_list[column], size=10, symbol=4, width=3,
                                  xaxis='x2', yaxis='y2')
            data1.append(trace1)
            trace2 = create_trace(ecGEMyield_list[substrate_name], ecGEMyield_list[column_list[column]],
                                  '%s_ecGEM' % column_list[column], color_list[len(column_list) + column], size=10,
                                  symbol=4, width=3, xaxis='x2', yaxis='y2')
            data1.append(trace2)
...
def get_min_enzyme_cost(model, dict_coeff):
    ...
    * dict_coeff: {reaction ID: coeffient}.
    ...
    """
    with model: # 避免直接修改原模型，所有操作仅在临时上下文中生效。
        # 两次调用 slim_optimize() 获取原始目标的最优值（假设为唯一解，上下界相同）。
        # 若模型存在多组最优解，此处可能需扩展为 (0.99 * opt_value, opt_value) 以保留灵活性。
        bounds = (model.slim_optimize(), model.slim_optimize())
        cons_obj = model.problem.Constraint(  # 添加约束，强制新优化问题的目标函数值（如生物量）不低于原最优值的下限。
            # 若原生物量最优值为 0.8，则约束为 0.8 ≤ Biomass ≤ 0.8，即严格保持原最优。
            model.objective.expression,
            lb=min(bounds), ub=max(bounds))
        model.add_cons_vars(cons_obj)

        dict_obj = dict()
        for r in model.reactions: # 遍历所有反应，筛选出 dict_coeff 中定义的反应。
            if r.id in list(dict_coeff.index):
                # print(dict_coeff.loc[r.id,'kcat_MW'])
                # 计算酶成本系数：1 / (kcat * MW)，其中 kcat 是催化常数，MW 是酶分子量。仅考虑正向反应变量（假设反向通量不消耗酶或已约束）。
        ...
        model.objective = model_obj # 通过 set_linear_coefficients 注入酶成本系数，构建目标：最小化总酶成本 = Σ (通量ᵢ / (kcatᵢ * MWᵢ))
    ...
    for glc_concentration in glc_concentration_list:
        with enz_model as growth_model:
            ...
            try:
                ...
                pass
            # 在代谢模型中，ATPM反应模拟细胞基础代谢活动（如膜电位维持、基础酶活性）所需的ATP消耗。
            # 模型可自由分配ATP，生物量增长率可能提高（资源被重定向到生长相关反应）。
            growth_model.reactions.get_by_id('ATPM').bounds = (0.0, 1000)
            growth_model.reactions.get_by_id(use_substrate).bounds = (-glc_concentration, 0)
            ...
            yield_cost_efficiency_df.loc[glc_concentration, 'enzyme efficiency'] = pfba_solution.fluxes[
                                                                                       obj] / enz_solution.objective_value
    ...
    * fig: go.Figure - Plotly Figure object.
    ...
    layout = go.Layout(
        ...
        yaxis2=dict(
            ...
            range=[np.min(efficiency_pfba['enzyme efficiency']) - 0.1,
                   np.max(efficiency_pfba['enzyme efficiency']) + 0.1],
            overlaying='y',
            tickfont=dict(color='black', size=20, family='Times New Roman'),
            ticks='inside',
            tickcolor='black',
...
def get_enz_foldchange(ecModel_file, obj, substrate, substrate_con, biomass_id, biomass_min, biomass_max, FC_threshold,
                       fluxes_outfile, enzcost_diff_file):
    '''
    ...
    * enzcost_diff: pd.DataFrame - DataFrame containing enzyme cost difference data.
    ...
    enzcost_diff.loc[valid_values, 'log2_foldchange(max/min)'] = np.log2(
        enzcost_max.loc[valid_values, 'E'] / enzcost_min.loc[valid_values, 'E'])
    enzcost_diff.loc[valid_values, 'log2_foldchange(min/max)'] = np.log2(
        enzcost_min.loc[valid_values, 'E'] / enzcost_max.loc[valid_values, 'E'])
...
def run_FSEOF(model, substrate, substrate_con, biomass_id, obj, FSEOF_file):
    ...
    * FESEOF_gene: pd.DataFrame - DataFrame containing genes and their regulations.
    ...
    unchanged = FSEOFdf_drop[
        (FSEOFdf_drop.iloc[:, 3:13] <= 1).all(axis=1) & (FSEOFdf_drop.iloc[:, 3:13] >= 0.95).all(axis=1)]
    ...
    FESEOF_gene = pd.concat([FSEOFdf_f['genes'], FSEOFdf_f['regulation'], FSEOFdf_f['reactions'], FSEOFdf_f['check']],
                            axis=1)
...
def get_reaction_kcatmw_onestop_by_AutoPACMEN(autopacmen_folder, sbml_path, bigg_metabolites_file, brenda_textfile_path,
                                              project_name, uniprot_data_file, organism, protein_kcat_database_path,
                                              kcat_gap_fill, reaction_gap_fill):
    # output files
...
def get_reaction_kcatmw_onestop_by_DLKcat(dlkcat_folder, sbml_path):
    ...
    if os.path.exists(DLouputdf_file):
        ...
        cmd_str = "python ./script/prediction_for_input.py %s/DLinput.tsv %s/DLoutput.tsv" % (
            dlkcat_folder, dlkcat_folder)
...
def get_reaction_kcatmw_onestop(work_folder, kcat_method, model_file, bigg_metabolites_file, brenda_textfile_path,
                                uniprot_data_file, organism, kcat_gap_fill, reaction_gap_fill):
    if kcat_method == 'AutoPACMEN':
        ...
        reaction_kcat_MW_file = get_reaction_kcatmw_onestop_by_AutoPACMEN(autopacmen_folder, model_file,
                                                                          bigg_metabolites_file, brenda_textfile_path,
                                                                          project_name, uniprot_data_file, organism,
                                                                          protein_kcat_database_path, kcat_gap_fill,
                                                                          reaction_gap_fill)
...
def visulization_phenotype_results(phenotypes, exp_col_namw, sim_col_name, x_max, y_max, out_figfile):
    ...
    for eachmet in list(phenotypes.index):
        trace0 = go.Scatter(name=eachmet, x=[phenotypes.loc[eachmet, exp_col_namw]],
                            y=[phenotypes.loc[eachmet, sim_col_name]],
    ...
    layout = go.Layout(
        xaxis=dict(title=dict(text="<b>Measured growth rate (h<sup>-1</sup>)<b>",
                              font=dict(size=30, family='Times New Roman')), range=[0, x_max],
                   tickfont=dict(color='black', size=15, family='Times New Roman')),
        yaxis=dict(title=dict(text="<b>Predicted growth rate (h<sup>-1</sup>)<b>",
                              font=dict(size=30, family='Times New Roman')), range=[0, y_max],
                   tickfont=dict(color='black', size=15, family='Times New Roman')),
...
def compare_phenotype_results(phenotypes, exp_col_name, sim1_col_name, sim1_name, sim2_col_name, sim2_name,
                              out_figfile):
    ...
    layout = go.Layout(
        xaxis=dict(title=dict(font=dict(size=30, family='Times New Roman')),
                   tickfont=dict(color='black', size=30, family='Times New Roman')),
        yaxis=dict(title=dict(text="<b>Estimation error<b>", font=dict(size=30, family='Times New Roman')),
                   tickfont=dict(color='black', size=30, family='Times New Roman')),
